"""Terminal front-end for the pre-cloud HITL gate that pauses the graph with the raw vs
sanitized content difference when requested."""
...
def render_chunks(query: dict | None, chunks: list[dict]) -> str:
...
def _menu_gate(payload: dict, banner: str | None = None) -> object:
...
def _stdin_gate(payload: dict) -> object:
    ...
    try:
        while True:
            print()
            for option in payload.get("options", []):
                marker = str_brand(f"[{option['choice']}]", bold=True)
